 ...
 */
 void Status_Leds_Init(void){
    GPIO_InitTypeDef led_a_init = {
        ...
        .Pull = GPIO_NOPULL,
        .Pin = (ARRAY_FAULT_LED_PIN | MOTOR_FAULT_LED_PIN | MOTOR_TIMEOUT_FAULT_LED_PIN)
      };
 ...
 void Status_Leds_Write(status_led_t led, bool state){
        switch(led){
            case MOTOR_FAULT_LED:
                HAL_GPIO_WritePin(MOTOR_FAULT_LED_PORT, MOTOR_FAULT_LED_PIN, state);
                break;
            ...
            case ARRAY_FAULT_LED:
                HAL_GPIO_WritePin(ARRAY_FAULT_LED_PORT, ARRAY_FAULT_LED_PIN, state);
                break;
 ...
 void Status_Leds_Toggle(status_led_t led){
    switch(led){
        case MOTOR_FAULT_LED:
            HAL_GPIO_TogglePin(MOTOR_FAULT_LED_PORT, MOTOR_FAULT_LED_PIN);
            break;
        case MOTOR_TIMEOUT_FAULT_LED:
            HAL_GPIO_TogglePin(MOTOR_TIMEOUT_FAULT_LED_PORT, MOTOR_TIMEOUT_FAULT_LED_PIN);
            break;
        case ARRAY_FAULT_LED:
            HAL_GPIO_TogglePin(ARRAY_FAULT_LED_PORT, ARRAY_FAULT_LED_PIN);
            break;
        case ARRAY_TIMEOUT_FAULT_LED:
